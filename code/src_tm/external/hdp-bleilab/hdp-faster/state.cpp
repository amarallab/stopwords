...
void HDP::save_state_labels(const char* name) {
  //hdp_state_->save_hdp_state(name);
  char filename[500];
  printf("hello");
  sprintf(filename, "%s.word-assignments", name);
  FILE* file = fopen(filename, "w");
    ...
    for (int d = 0; d < num_docs_; d++)
    {
        DocState* d_state = doc_states_[d];
        int doc_id = d_state->doc_id_;
        for (int i = 0; i < d_state->doc_length_; i++)
        {
            w = d_state->words_[i].word_;
            z = d_state->words_[i].topic_assignment_;
            fprintf(file, "%d %d %d\n",
                    doc_id, w, z);
        }
    }
